...
function deployFarm() {
  ...
  scripts/deployFarm.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
function upgradeFarm() {
  ...
  scripts/upgradeFarm.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    sudo rm -rf channel-artifacts log.txt farm.tar.gz
    sudo rm -rf farm_*
...
if [ "$MODE" == "up" ]; then
  ...
  echo
elif [ "$MODE" == "deployFarm" ]; then
  echo "Deploying chaincode on channel '${CHANNEL_NAME}'"
  echo
elif [ "$MODE" == "upgradeFarm" ]; then
  echo "UpgradeFarm Farm chaincode on channel '${CHANNEL_NAME}'"
  echo
else
  printHelp
  exit 1
